...
use Numbers\Backend\ABAC\DataSource\AttributesWithModels;
...
class Attributes extends Table
{
    ...
    public function preloadModelsAndFields()
    {
        ...
        if (!isset($this->attribute_all_models)) {
            $this->attribute_all_models = AttributesWithModels::getStatic();
        }
    }
    ...
    public function formProcessWidgetData(& $form, $parent_keys, $data, $parent_data, $fields, $options)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            // if we have no data
            if (($v['wg_attribute_attribute_id'] ?? '') == '' && ($v['wg_attribute_value'] ?? '') == '') {
                continue;
            }
            ...
            if (empty($this->attribute_all_fields[$id])) {
                ...
                $type_options = Common::processDomainsAndTypes($type_options);
    ...
    public function overrideFieldValue(& $form, & $options, & $value, & $neighbouring_values)
    {
        ...
        if (empty($neighbouring_values['wg_attribute_attribute_id']) || empty($this->attribute_all_fields[$neighbouring_values['wg_attribute_attribute_id']])) {
            ...
            $temp = Common::processDomainsAndTypes(['options' => $options['options']]);
            ...
            } elseif (in_array($field['tm_attribute_method'], ['select', 'multiselect', 'autocomplete', 'multiautocomplete'])) {
